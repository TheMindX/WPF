...
namespace TimeSheet
{
    ...
    /// AppControl.xaml 
    /// </summary>
    public partial class ExternalAppControl : UserControl, IDisposable
    {
        ...
        protected void OnSizeChanged(object s, SizeChangedEventArgs e)
        {
            if (this._appWin != IntPtr.Zero)
            {
                ...
                MoveWindow(_appWin, (int)p.X, (int)p.Y, (int)this.ActualWidth, (int)this.ActualHeight, true);
            }
        }

        Point getWindowPosition()
        {
            FrameworkElement ui = this;
            while (ui.Parent != null)
            {
                ui = ui.Parent as FrameworkElement;
            }
            Point relativePoint = this.TransformToAncestor(ui).Transform(new Point(0, 0));
